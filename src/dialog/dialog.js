function dialog(option){
	option = option || {};

	var defaults = {
		title:"这是一个弹框",
		content:"我是弹框",
		okFn:function(){}
	};		

	for( var attr in option ){
		defaults[attr] = option[attr];
	}

	//封装弹框
	//弹框的结构，appendChild到body中

	var diaDiv = document.createElement("div");
	diaDiv.className = "full-pop";

	var diaHtml = `<h5>
		<p class="Title">${defaults.title}</p>
		<a href="javascript:void(0);" class="close" title="关闭">×</a>
	</h5>
	<div class="content">
		${defaults.content}
	</div>
	<div class="pop-btns">
		<span class="error"></span>
		<a href="javascript:void(0)" class="confirm">确定</a>
		<a href="javascript:void(0)" class="cancel">取消</a>
	</div>`;

	diaDiv.innerHTML = diaHtml;

	document.body.appendChild(diaDiv);

	diaDiv.style.zIndex = 100;

	//遮罩

	var mask = document.createElement("div");

	mask.style.cssText = "width:100%;height:100%;background:#000;opacity:0;position:fixed;left:0;top:0;z-index:99;";
	$(mask).animate({'opacity':0.5},800);
	document.body.appendChild(mask);


	//居中显示
	
	diaDiv.style.left = (document.documentElement.clientWidth - diaDiv.offsetWidth)/2 + "px";
	diaDiv.style.top ='1000px';
var left=(document.documentElement.clientWidth - diaDiv.offsetWidth)/2 ;
var top=(document.documentElement.clientHeight - diaDiv.offsetHeight)/2
$(diaDiv).animate({'top':top,'left':left},800,'swing')
	
	window.addEventListener("resize",function (){
		diaDiv.style.left = (document.documentElement.clientWidth - diaDiv.offsetWidth)/2 + "px";
		diaDiv.style.top = (document.documentElement.clientHeight - diaDiv.offsetHeight)/2 + "px";		
	},false);

//给确定取消关闭添加点击处理

	var close = diaDiv.querySelector(".close");
	var ok = diaDiv.querySelector(".confirm");
	var cancel = diaDiv.querySelector(".cancel");

	close.addEventListener("click",function (){
		$(diaDiv).animate({'top':-800},500);
			$(mask).animate({'opacity':0},500);
			setTimeout(function(){
				document.body.removeChild(diaDiv);
				document.body.removeChild(mask);
				mask=null;
				diaDiv=null;
			},500)
	},false)
	ok.addEventListener("click",function (){

		var bl = defaults.okFn(); 
		/*
			1. return undefined 关闭
			2. return false  关闭
			3. return true   不关闭
		*/

		if( !bl ){
			$(diaDiv).animate({'top':-800},500);
			$(mask).animate({'opacity':0},500);
			setTimeout(function(){
				document.body.removeChild(diaDiv);
				document.body.removeChild(mask);
				mask=null;
				diaDiv=null;
			},500)
		}

		
	},false)
	cancel.addEventListener("click",function (){
		$(diaDiv).animate({'top':-800},500);
			$(mask).animate({'opacity':0},500);
			setTimeout(function(){
				document.body.removeChild(diaDiv);
				document.body.removeChild(mask);
				mask=null;
				diaDiv=null;
			},500)
	},false)
}